...
> 📖 **처음이신가요? [사용설명서](docs/사용설명서.md)를 보세요.**
> (사이트 안에서도 `0 시작` 화면의 "사용설명서 열기" 버튼으로 볼 수 있습니다.)
> 주식·AI·통계를 전혀 모르는 사람 기준으로 화면 하나하나, 숫자 하나하나를 설명해 둔 문서입니다.
...
그 뒤로는 전부 자동입니다. `main` 의 `docs/` 가 바뀌면 `pages.yml` 이 다시 올립니다.

데이터 갱신·모델 학습 워크플로가 `docs/data/*.json` 을 커밋할 때는 조금 다른 길을 씁니다.
GitHub 은 **워크플로 토큰으로 만든 push 로는 다른 워크플로를 트리거하지 않기 때문**입니다
(워크플로가 서로를 끝없이 부르는 사고를 막는 안전장치). 그래서 `pages.yml` 은 push 대신
`workflow_run` 으로 "그 워크플로가 끝났다"는 신호를 직접 받아 이어서 배포합니다.
...
- [x] **배포** — GitHub Pages 로 공개. 데이터·모델 갱신이 사이트 재배포까지 연결됨
- [x] **사전 학습 전체 실행** — LSTM·GRU·TCN·Transformer·EGARCH·LSTM-Vol·HMM 7종,
      5종목 × 1404일 예측. **등록 모델 30개가 전부 실행 가능**
...
(브라우저에서 **30개 모델 전부** 실행, 24초)

**방향 예측** — 20개 (기준선 6개 포함, `*` 표시)

| 순위 | 모델 | 실행 | 총점 | 예측력 | 성과 | 안정성 | 실용성 | AUC | 승률 | p |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Ridge 회귀 | 실시간 | 24.1 | 16.1 | −4.4 | 28.2 | 90 | 0.5081 | 3/5 | 0.60 |
| — | 모멘텀 규칙 * | 실시간 | 15.5 | −12.8 | −20.3 | 47.6 | 90 | 0.4936 | 3/5 | 0.68 |
| 3 | k-최근접 이웃 | 실시간 | 14.0 | 14.1 | −4.0 | 18.8 | 42 | 0.5071 | 2/5 | 0.65 |
| — | 바이앤홀드 * | 실시간 | 13.4 | 0 | −0.3 | 0 | 90 | — | 0/5 | — |
| 8 | 신경망(MLP) | 실시간 | 8.2 | **26.4** | −46.7 | **82.6** | −42 | 0.5132 | 5/5 | 0.39 |
| 9 | 소형 Transformer | 사전 | 4.8 | −7.3 | **+4.7** | 16.5 | 10 | 0.4964 | 2/5 | 0.81 |
| 16 | LSTM | 사전 | −23.7 | −43.2 | −45.6 | 5.9 | 10 | 0.4784 | 1/5 | 0.16 |
| 19 | 1D-CNN / TCN | 사전 | −31.9 | −48.7 | −68.1 | 6.8 | 10 | 0.4756 | 1/5 | 0.11 |
| 20 | GRU | 사전 | −32.3 | −39.6 | −87.0 | 16.8 | 10 | 0.4802 | 2/5 | 0.20 |

**변동성 예측** — 7개, 순위는 방향 예측과 따로

| 순위 | 모델 | 실행 | 총점 | 예측력 | 안정성 | 실용성 | MSE | 승률 |
|---|---|---|---|---|---|---|---|---|
| 1 | EGARCH | 사전 | **42.7** | 40.2 | 41.3 | 50 | 6.96e-7 | 3/5 |
| 2 | HAR-RV | 실시간 | 39.6 | 5.2 | **55.5** | 82 | 7.49e-7 | 3/5 |
| 3 | RF-Vol | 실시간 | 28.7 | 9.4 | 53.1 | 27 | 7.43e-7 | 3/5 |
| 4 | GJR-GARCH | 실시간 | 28.6 | **43.2** | 18.9 | 15 | 6.92e-7 | 2/5 |
| — | EWMA * | 실시간 | 19.3 | 0 | 0 | 90 | 7.57e-7 | 0/5 |
| 6 | LSTM-Vol | 사전 | 14.6 | −6.3 | 42.5 | 10 | 7.67e-7 | 3/5 |

`*` = 기준선. 기준선이 자기 축에서 정확히 0점을 받는 것으로 점수 체계가 검증됩니다.

**읽는 법**

- **방향 예측에서 딥러닝이 전부 하위권입니다.** LSTM·GRU·TCN의 AUC가 0.476~0.480으로
  동전 던지기(0.5)보다도 낮습니다. "AI 모델을 쓰면 주가를 맞힐 수 있다"는 기대에 대한
  이 사이트의 정직한 답입니다. Transformer만 수익성 축에서 유일하게 플러스(+4.7)인데,
  AUC는 0.496이라 예측력이 아니라 **거래를 덜 해서** 얻은 결과입니다.
- **p값이 하나도 0.05 밑으로 안 내려갑니다.** 20개 중 우연을 넘어선 모델이 없습니다.
- 반대로 **변동성은 실제로 예측됩니다.** 사전 학습 EGARCH가 1위(총점 42.7). 원시 MSE는
  GJR-GARCH가 근소하게 더 낮은데도 EGARCH가 앞서는 이유는 폴드별로 덜 흔들리기 때문입니다.
- MLP는 예측력·안정성 1위인데 총점 8위입니다. 느리고, 설명이 안 되고, 거래비용을 물면
  남지 않아서입니다. **한 지표만 보면 안 되는 이유**가 이 한 줄에 다 들어 있습니다.

**한 줄 결론** (사이트가 자동 생성): *방향 예측엔 신경망(MLP), 수익성엔 소형 Transformer,
변동성 예측엔 GJR-GARCH*
